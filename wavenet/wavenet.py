...
class WaveNet(nn.Module):
    ...
    def __init__(self, out_channels=256, layers=20, stacks=2,
        ...
        self.condition_conv_layers = nn.ModuleList([
            Conv1d1x1(cin_channels, gate_channels,
                      weight_normalization=weight_normalization),
            nn.Tanh(),
            Conv1d1x1(gate_channels, gate_channels,
                      weight_normalization=weight_normalization),
            nn.Tanh(),
            Conv1d1x1(gate_channels, skip_out_channels,
                      weight_normalization=weight_normalization),
            nn.Tanh(),
        ])
        self.last_conv_layers = nn.ModuleList([
            ...
            Conv1d1x1(gate_channels, gate_channels,
                      weight_normalization=weight_normalization),
            nn.Tanh(),
            Conv1d1x1(gate_channels, out_channels,
                      weight_normalization=weight_normalization),
            nn.Tanh(),
        ...
        for f in self.conv_layers:
            ...
            if skips is None:
                ...
                if self.legacy:
                    skips *= math.sqrt(0.5)
        cx=c
        for f in self.condition_conv_layers:
            cx = f(cx)
        
        x = torch.cat((skips,cx),1)
    ...
    def incremental_forward(self, initial_input=None, c=None, g=None,
        ...
        for t in tqdm(range(T)):
            ...
            for f in self.condition_conv_layers:
                cx = f(cx)
            # print(cx.size(),skips.size())   
            x = torch.cat((skips,cx.transpose(1, 2)),1)
            # print(x.size())
            for f in self.last_conv_layers:
                try:
